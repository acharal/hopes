...
data ComputedAnswer = Computed Subst [CExpr]
...
instance Pretty ComputedAnswer where
    ppr (Computed s e) = vcat [ pprsubst s, ppr_expr' e ]
        where pprsubst  xs  = vcat $ map (ppr_bind "=" []) xs
              ...
              aux (CNot e) =
                let (v', CEq t1 t2) = splitExist e
                in ppr_bind "/=" (namedVars v') (t1, t2)
...
ppr_expr vs e@(CApp _ _ _) = ppr (functor e) <> parens (sep (map (\as -> parens (sep $ punctuate comma (map (ppr_expr vs) as))) (args e)))
ppr_expr vs e@(CExists _ _ _) =
    let (vs', e') = splitExist e
    in ppr_expr (vs ++ (namedVars vs')) e'
ppr_expr vs e = ppr_basic' e


data BasicTuple = BasicTuple [VarSym] [BasicExprRep]
    deriving Show

data BasicExprRep =
    BasicSet { pos  :: [BasicTuple]   -- a set of positive tuples
             , neg  :: [BasicTuple]   -- a set of negative tuples
             , vars :: [CExpr]        -- ground representation of atom
             }
    | BasicGround CExpr               --
    deriving Show

instance Monoid (BasicExprRep) where
    mempty  = BasicSet [] [] []
...
mkbasic e =
    let splitAnd (CAnd _ b1 b2) = splitAnd b1 ++ splitAnd b2
        splitAnd e = [e]

        splitAndOr (CAnd _ b1 b2) = splitAndOr b1 ++ splitAndOr b2
        splitAndOr (COr  _ b1 b2) = splitAndOr b1 ++ splitAndOr b2
        splitAndOr e = [e]

        simple e =
          let simple' [] e        = mkvar e
              simple' vs (CNot e) = mkneg (tup vs e)
              simple' vs e        = mkpos (tup vs e)
              (vs, e') = splitLambda e
          in simple' (concat vs) e'

        tup vs e = BasicTuple v' es
            where (vf', e') = splitExist e
                  v' =  namedVars vf'
                  es = map snd $ sortBy (o vs) $ q $ groupBy v $ map tupElem $ splitAnd e'
--                  tupElem :: Expr a -> (a, Either (BasicExprRep a) (BasicTuple a))
                  tupElem (CEq (CVar x) e) = (x, Left (mkgrd e))
                  tupElem e@(CApp _ _ _) =
                    case functor e of
                        CVar x -> (x, Right $ BasicTuple [] (map mkbasic (concat (args e))))
                        _ -> error "tupElem"
                  v (x, _) (y, _) = x == y
                  q xs = map q'' $ map q' xs
                  q' [(x,y)] = (x, [y])
...
instance Pretty BasicExprRep where
    ppr e = ppr' [] e
        where ppr' :: [VarSym] -> BasicExprRep -> Doc
              ppr' vs (BasicGround e)  = ppr_expr vs e
              ppr' vs (BasicSet p n v) = (pprset vs p) <+> text "except" <+> (pprset vs n)
              pprset :: [VarSym] -> [BasicTuple] -> Doc
